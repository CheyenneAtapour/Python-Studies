# adapted from https://github.com/man-c/pycoingecko/blob/master/pycoingecko/utils.py
...
cg = CoinGeckoAPI()
prev = 0.185644 # enj = 0.19
start = 0.185644

while True:
    time.sleep(1)
    dict = cg.get_price('enjincoin', 'usd') # {'enjincoin': {'usd': ####}}
    enjprice = dict['enjincoin']['usd']
    print(enjprice)
    if enjprice - prev > .002 or enjprice - start > .002:
        print ("the price started going up")
        break
    if enjprice - prev > 0:
        print ("up")
    prev = enjprice